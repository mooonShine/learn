...
class Component_SubPages {
    ...
    public function __construct($each_disNums, $nums, $current_page, $sub_pages, $subPage_link,$subPage_type = 'backend', $is_ajax = false) {
    ...
    function subPageCss() {
        $subPageCssStr = "";
        switch ($this->subPage_type) {
            case 'front':
                $subPageCssStr = $this->subPageCssFront();
                break;
            case 'backend':
                $subPageCssStr = $this->subPageCssBackend();
                break;
            default:
                $subPageCssStr = $this->subPageCssBackend();
                break;
     ...
     * <span>|</span>
                ...
                <span>|</span>
     */
    private function subPageCssFront() {
        
        $subPageCssStr = "<span>|</span>";

        $a = $this->construct_num_Page();
        for ($i = 0; $i < count($a); $i++) {
            $s = $a[$i];
            if ($s == $this->current_page) {
                $subPageCssStr .= "<span class='current'>".$s."</span>";
            } else {
    ...
    //后台分页
    private function subPageCssBackend() {
        $subPageCssStr = "";
